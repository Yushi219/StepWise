...
function ChModelFirst() {
  ...
  const steps = [
    ...
    { t: ["Define the typical rule", "定义典型规则"], d: ["Run width, landing, railing, support — clear, not nested dialogs.", "梯段宽度、平台、栏杆、支撑——清晰，而非嵌套对话框。"] },
    { t: ["Preview the stair stack", "预览楼梯堆叠"], d: ["A non-committed preview builds level by level.", "未提交的预览，逐层搭建。"] },
    { t: ["Mark exceptions", "标记例外"], d: ["L10 / L20 equipment floors become local overrides — not duplicate types.", "L10 / L20 设备层成为本地覆盖——而非复制类型。"] },
    { t: ["Review impact & apply", "审阅影响后提交"], d: ["See affected runs, landings, railings and views — then Review Impact, not silent Apply.", "先看受影响的梯段、平台、栏杆与视图——再进入 Review Impact，而非静默 Apply。"] },
  ];
